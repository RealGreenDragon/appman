...
class Git_Manager(Base_Profile):

    program_name = 'Git'

    default_path = 'C:\\Portable\\git\\'
    ...
    def _get_latest_version(self):
        ...
        Returns:
            str : Latest version
        """
        with self._http_head_req('https://github.com/git-for-windows/git/releases/latest') as r:
    ...
    def _get_download_data(self):
        ...
        """
        url = 'https://github.com/git-for-windows/git/releases/download/v{long_ver}/PortableGit-{short_ver}-{arch}-bit.7z.exe' \
        .format(
            long_ver=self._latest_version,
            short_ver=self._latest_version.replace('.windows.1', '').replace('.windows', ''),
            arch=self._arch
            )
        return dl_get(
                os.path.join(self._tmp_dir, 'git_latest.7z'),
                url
            )
    ...
    def _update_program(self):
        ...
        # Change cwd
        t = self._get_cwd()
        self._set_cwd(self._path)

        # Fix 'post-install.bat'
        f = self._load_file('post-install.bat')
        self._save_file(
            f.replace('@DEL post-install.bat', '@REM post-install.bat'),
            'post-install.bat'
            )

        # Exec 'post-install.bat'
        self._cmd_exec('cmd /c post-install.bat')
