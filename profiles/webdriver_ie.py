...
class WebdriverIE_Manager(Base_Profile):

    program_name = 'Webdriver-IE'

    default_path = 'C:\\Portable\\webdrivers\\ie\\'
    ...
    def _get_latest_version(self):
        ...
        # Get major version (fix numbers to order them correctly)
        index_page = self._http_get_req('https://selenium-release.storage.googleapis.com/?delimiter=/&prefix=')
        raw_versions = [x.split('/</Prefix>')[0] for x in index_page.split('<CommonPrefixes><Prefix>')[1:]]
        filtered_versions = sorted([int(raw_v.replace('.','')) for raw_v in raw_versions if is_float(raw_v)])
        final_versions = [str(f_v)[0] + '.' + str(f_v)[1:] for f_v in filtered_versions]
        major_ver = final_versions[-1]

        # Get full version
        version_page = self._http_get_req('https://selenium-release.storage.googleapis.com/?delimiter=/&prefix={}/'.format(major_ver))
        full_versions = [x.split('.zip')[0] for x in version_page.split('IEDriverServer_Win32_')[1:]]
        full_version = full_versions[-1]

        # Format direct url
        self.direct_url = 'https://selenium-release.storage.googleapis.com/{}/IEDriverServer_{}_{}.zip' \
        .format(major_ver, 'Win32' if self._arch == '32' else 'x64', full_version)
    ...
    def _get_download_data(self):
        ...
        return dl_get(
                os.path.join(self._tmp_dir, 'webdriver_ie_latest.zip'),
                self.direct_url
            )
