...
class GhostScript_Manager(Base_Profile):

    program_name = 'GhostScript'

    default_path = 'C:\\Portable\\gs\\'
    ...
    def _get_latest_version(self):
        ...
        Returns:
            str : Latest version
        """
        with self._http_head_req('https://github.com/ArtifexSoftware/ghostpdl-downloads/releases/latest') as r:
            return r.headers['Location'].split('/gs')[-1].strip()
    ...
    def _get_download_data(self):
        ...
        url = 'https://github.com/ArtifexSoftware/ghostpdl-downloads/releases/download/gs{vers}/gs{vers}w{arch}.exe' \
        .format(vers=self._latest_version, arch=self._arch)
        return dl_get(
                os.path.join(self._tmp_dir, 'gs_latest.zip'),
                url
            )
    ...
    def _update_program(self):
        ...
        self._delete_file(os.path.join(self._path, 'uninstgs.exe.nsis'))
