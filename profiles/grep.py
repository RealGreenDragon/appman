...
class Grep_Manager(Base_Profile):

    program_name = 'Grep'

    default_path = 'C:\\Portable\\unixtools\\grep\\'
    ...
    def _get_latest_version(self):
        ...
        last_ver = page_html.split('<td valign="top"><a href="packages/grep')[1] \
    ...
    def _get_download_data(self):
        ...
        """
        url = self._http_get_req('http://gnuwin32.sourceforge.net/downlinks/grep.php').split('; url=')[1].split('"', 1)[0]
        return dl_get(
                os.path.join(self._tmp_dir, 'grep_latest.exe'),
                url
            )
