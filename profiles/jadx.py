...
class JadX_Manager(Base_Profile):

    program_name = 'JadX'

    default_path = 'C:\\Portable\\jadx\\'
    ...
    def _get_latest_version(self):
        ...
        Returns:
            str : Latest version
        """
        with self._http_head_req('https://github.com/skylot/jadx/releases/latest') as r:
    ...
    def _get_download_data(self):
        ...
        url = 'https://github.com/skylot/jadx/releases/download/v{vers}/jadx-{vers}.zip' \
        .format(vers=self._latest_version)
        return dl_get(
                os.path.join(self._tmp_dir, 'jadx_latest.zip'),
                url
            )
    ...
    def _install_program(self):
        ...
        '''
        # NOT NEEDED
        # Edit bat files to allow the call from any location
        bin_path = os.path.join(self._path, 'bin') + '\\'
        for bat in (os.path.join(bin_path, 'jadx.bat'), os.path.join(bin_path, 'jadx-gui.bat')):
            f = self._load_file(bat)
            self._save_file(f.replace('set DIRNAME=%~dp0', 'set DIRNAME=' + bin_path), bat)
        '''
