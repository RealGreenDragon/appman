...
class MKVToolNix_Manager(Base_Profile):
    ...
    program_name = 'MKVToolNix'

    default_path = 'C:\\Portable\\mkvtoolnix\\'
    ...
    def _get_download_data(self):
        ...
        url = 'https://mkvtoolnix.download/windows/releases/{vers}/mkvtoolnix-{arc}-bit-{vers}.7z' \
        .format(vers=self._latest_version, arc=self._arch)
        return dl_get(
                os.path.join(self._tmp_dir, 'mkvtoolnix_latest.7z'),
                url
            )
    ...
    def _extract_latest_version(self):
        ...
        self._extract(self._dl_data_list[0].path, 'C:\\Portable')
