...
class NMap_Manager(Base_Profile):

    program_name = 'NMap'

    default_path = 'C:\\Portable\\nmap\\'
    ...
    def _get_latest_version(self):
        ...
        return self._http_get_req(
            'https://nmap.org/download.html'
            ).split('self-installer: <a href="https://nmap.org/dist/nmap-')[1].split('-setup.exe"')[0].strip()
    ...
    def _get_download_data(self):
        ...
        return dl_get(
                os.path.join(self._tmp_dir, 'nmap_latest.zip'),
                'https://nmap.org/dist/nmap-{}-win32.zip'.format(self._latest_version)
            )
    ...
    def _update_program(self):
        ...
        extracted_folder = os.path.join(
            self._path,
            'nmap-{}'.format(self._latest_version)
            )
