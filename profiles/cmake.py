...
class MKVToolNix_Manager(Base_Profile):
    ...
    def _get_latest_version(self):
        ...
        return self._http_get_req('https://cmake.org/download/').split('Latest Release (')[1].split(')')[0].strip()
    ...
    def _get_download_data(self):
        ...
        branch_version = self._latest_version.rsplit('.', 1)[0]
        if not '.' in branch_version:
            branch_version = self._latest_version
        self.cmake_name = 'cmake-{}-win{}-x{}'.format(
            self._latest_version, self._arch, '86' if self._arch=='32' else '64')

        return dl_get(
                os.path.join(self._tmp_dir, 'cmake_latest.zip'),
                'https://cmake.org/files/v{}/{}.zip'.format(branch_version, self.cmake_name)
            )
    ...
    def _update_program(self):
        ...
        extracted_folder = os.path.join(self._path, self.cmake_name)
