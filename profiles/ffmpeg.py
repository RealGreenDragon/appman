...
class FFmpeg_Manager(Base_Profile):

    program_name = 'FFmpeg'

    default_path = 'C:\\Portable\\ffmpeg\\'
    ...
    def _get_latest_version(self):
        ...
        STABLE = 'Release builds are recommended for distributors, but cannot be used when submitting bugs.'
        SNAPSHOT = 'Nightly git builds contain more features, are usually stable, and are the required version when submitting bugs.'

        return self._http_get_req('http://ffmpeg.zeranoe.com/builds/').split(STABLE)[1].split('</label>')[0].split('>')[-1].strip()
    ...
    def _get_download_data(self):
        ...
        url = 'http://ffmpeg.zeranoe.com/builds/win{arch}/static/ffmpeg-{name}-win{arch}-static.zip' \
                    .format(name=self._latest_version, arch=self._arch)
        return dl_get(
                os.path.join(self._tmp_dir, 'ffmpeg_latest.zip'),
                url
            )
    ...
    def _update_program(self):
        ...
        extracted_folder = os.path.join(
            self._path,
            'ffmpeg-{}-win{}-static'.format(self._latest_version, self._arch)
            )
