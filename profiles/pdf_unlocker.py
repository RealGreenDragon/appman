...
class PDFUnlocker_Manager(Base_Profile):

    program_name = 'pdf-unlocker'

    default_path = 'C:\\Portable\\pdfunlocker\\'
    ...
    def __init__(self, **prog_data):
        ...
        self.main_exe = os.path.join(self._path, 'Portable PDF Unlocker.exe')
        self.new_main_exe = os.path.join(self._path, 'pdf-unlocker.exe')
    ...
    def _get_download_data(self):
        ...
        url = self._http_get_req('http://sourceforge.net/projects/portablepdfunlocker/files/Portable%20PDF%20Unlocker%20x86%20x64%20{}.7z/download' \
        .format(self._latest_version)).split('; url=')[1].split('"')[0]
        return dl_get(
                os.path.join(self._tmp_dir, 'pdfunlocker_latest.rar'),
                url
            )
    ...
    def _extract_latest_version(self):
        ...
        self._rename(self.main_exe, self.new_main_exe)
