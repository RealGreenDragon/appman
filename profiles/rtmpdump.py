...
class RTMPDump_Manager(Base_Profile):

    program_name = 'RTMPDump'

    default_path = 'C:\\Portable\\rtmpdump\\'
    ...
    def _get_latest_version(self):
        ...
        page_html = self._http_get_req('https://www.videohelp.com/software/RTMPDump')
        last_ver = page_html.split('<title>RTMPDump ')[1].split(' Download')[0]
        self.dl_url = page_html.split('Download RTMPDump ' + last_ver)[0].split('<a href="')[-1].split('"')[0]
        return last_ver
    ...
    def _get_download_data(self):
        ...
        """
        url = self._http_get_req(self.dl_url).split('">Download rtmpdump-')[0].split('href="')[-1]
        return dl_get(
                os.path.join(self._tmp_dir, 'rtmpdump_latest.zip'),
                url
            )
