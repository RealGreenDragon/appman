...
class QPDF_Manager(Base_Profile):

    program_name = 'qpdf'

    default_path = 'C:\\Portable\\qpdf\\'
    ...
    def _get_latest_version(self):
        ...
        Returns:
            str : Latest version
        """
        with self._http_head_req('https://github.com/qpdf/qpdf/releases/latest') as r:
    ...
    def _get_download_data(self):
        ...
        url = 'https://github.com/qpdf/qpdf/releases/download/release-qpdf-{ver}/qpdf-{ver}-bin-mingw{arc}.zip' \
        .format(ver=self._latest_version, arc=self._arch)
        return dl_get(
                os.path.join(self._tmp_dir, 'qpdf_latest.zip'),
                url
            )
    ...
    def _update_program(self):
        ...
        extracted_folder = os.path.join(
            self._path,
            'qpdf-{}'.format(self._latest_version)
            )
